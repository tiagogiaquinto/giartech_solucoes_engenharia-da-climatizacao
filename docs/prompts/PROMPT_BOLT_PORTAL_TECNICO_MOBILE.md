# PROMPT — PORTAL DO TÉCNICO (MOBILE FIRST)
# Cole no Bolt.new e execute sem perguntar
# Foco: celular, uma mão, operação em campo, rápido e visual

## PRINCÍPIOS DE DESIGN MOBILE
- Botões mínimo 48px de altura (touch target confortável)
- Fonte mínima 15px — legível ao sol em campo
- Fundo escuro por padrão (economiza bateria OLED, legível ao sol)
- Zero scroll horizontal
- Ações principais sempre visíveis, nunca em menu escondido
- Status em cores grandes e óbvias — sem precisar ler o texto
- Abrir uma OS com 1 toque, executar a ação principal com 2 toques
- Nenhum valor financeiro em lugar algum

---

## CRIAR ARQUIVO: src/pages/TechnicianPortal.tsx
...
const Icon = {
  ...
  box:      () => <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={2} strokeLinecap="round" strokeLinejoin="round" className="w-4 h-4"><path d="M21 16V8a2 2 0 0 0-1-1.73l-7-4a2 2 0 0 0-2 0l-7 4A2 2 0 0 0 3 8v8a2 2 0 0 0 1 1.73l7 4a2 2 0 0 0 2 0l7-4A2 2 0 0 0 21 16z"/></svg>,
...
export default function TechnicianPortal() {
  ...
  // ─── LISTA PRINCIPAL ─────────────────────────────────────────
  const tabs: { id:Tab; icon:()=>JSX.Element; label:string; badge?:number }[] = [
              ...
              return (
                <button key={o.id} onClick={()=>abrirOS(o)}
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-sm font-semibold" style={{color:st.text}}>
                      {/* Status badges usam fundo branco para legibilidade */}
...
// na lista de rotas:
{ path: '/tecnico', element: <TechnicianPortal /> },
```

## REDIRECIONAR TÉCNICO NO LOGIN

Após autenticação, antes de redirecionar para /dashboard:
```tsx
...
const isTecnico = ['technician','tecnico'].includes(account?.role || '')
navigate(isTecnico ? '/tecnico' : '/dashboard')
```

## BANCO — TUDO JÁ EXISTE
Tabelas: `material_requests`, `material_request_items`
Funções: `get_os_tecnico`, `get_agenda_tecnico`, `get_tasks_tecnico`,
`get_inventario_tecnico`, `criar_solicitacao_material`,
`tecnico_atualizar_os`, `get_checklist_os_tecnico`, `tecnico_marcar_checklist`

## NOTAS DE DESIGN MOBILE
- Fundo escuro (#030712) — legível ao sol, economia de bateria
- Todos os botões de ação têm mínimo 48px de altura (h-12 a h-16)
- Ligar e Abrir no Maps são links nativos `tel:` e `https://maps.google.com`
- Modal de solicitação é um bottom sheet — sobe de baixo, natural no celular
- Barra de progresso tem 12px de altura — fácil de ler em campo
- Slider do progresso tem `step=5` — não precisa de precisão
- Sem nenhum valor financeiro em nenhuma tela
