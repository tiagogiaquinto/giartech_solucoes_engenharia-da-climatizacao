...
const Chat = () => {
  ...
  const [conversations, setConversations] = useState<Conversation[]>([
    ...
    },
    {
      ...
      lastMessage: 'Vou precisar de mais peças para o conserto',
    ...
    },
    {
      ...
      unreadCount: 0,
      isGroup: true,
      participants: ['Lucas', 'Fernanda', 'Rafael'],
      status: 'online'
    }
  ]);
              ...
              <div className="flex items-center space-x-2">
                ...
                {isMobile && (
                  <button 
                    ...
                    className="p-2 text-gray-500 hover:text-gray-700 hover:bg-gray-100 rounded-full md:hidden"
                  >
                        ...
                        {conversation.avatar ? (
                          ...
                          <div className={`w-12 h-12 rounded-full flex items-center justify-center ${
                            ...
                            {conversation.isGroup ? (
                              ...
                              <span className="text-lg font-semibold">{conversation.name.charAt(0)}</span>
                            )}
                          </div>
                        )}
              ...
              {showSettings && (
                ...
                >
                  ...
                  <div className="flex-1 overflow-y-auto p-4 space-y-6">
                    ...
                    <div>
                      <h3 className="text-sm font-medium text-gray-700 mb-3">Notificações</h3>
                      <div className="space-y-3">
                        <div className="flex items-center justify-between">
                          <span className="text-gray-600">Notificações de mensagens</span>
                        ...
                        <div className="flex items-center justify-between">
                          <span className="text-gray-600">Sons de notificação</span>
                          <label className="relative inline-flex items-center cursor-pointer">
                            <input 
                              type="checkbox" 
                    ...
                    {/* Privacy Settings */}
                    <div>
                      <h3 className="text-sm font-medium text-gray-700 mb-3">Privacidade</h3>
                      <div className="space-y-3">
                        <div className="flex items-center justify-between">
                          <span className="text-gray-600">Confirmações de leitura</span>
                          <label className="relative inline-flex items-center cursor-pointer">
                            <input 
                              type="checkbox" 
                    ...
                    {/* Appearance Settings */}
                    <div>
                      <h3 className="text-sm font-medium text-gray-700 mb-3">Aparência</h3>
                      <div className="space-y-3">
                        <div className="flex items-center justify-between">
                        ...
                        <div>
                          <span className="text-gray-600 block mb-2">Tamanho da fonte</span>
                          <select
      ...
      <div className="flex-1 flex flex-col">
        {activeConversation ? (
            ...
            <div className="p-4 border-b border-gray-200 flex items-center justify-between">
              <div className="flex items-center">
                {isMobile && (
                  <button 
                    onClick={toggleSidebar}
                    className="p-2 text-gray-500 hover:text-gray-700 hover:bg-gray-100 rounded-full mr-2 md:hidden"
                  >
                ...
                <div className="flex items-center space-x-3">
                  ...
                  {activeConversation.avatar ? (
                    ...
                    <div className={`w-10 h-10 rounded-full flex items-center justify-center ${
                      ...
                      {activeConversation.isGroup ? (
                        ...
                        <span className="text-lg font-semibold">{activeConversation.name.charAt(0)}</span>
                      )}
                    </div>
                  )}
                  ...
                  <div>
                    ...
                    <div className="flex items-center text-xs text-gray-500">
                      <div className={`w-2 h-2 rounded-full ${getStatusColor(activeConversation.status)} mr-1`}></div>
                      <span>
                      ...
                      {activeConversation.isGroup && (
                        <span className="ml-2">{activeConversation.participants.length} participantes</span>
                      )}
                    </div>
                  </div>
                </div>
                ...
                return (
                  <React.Fragment key={message.id}>
                    ...
                    <div className={`flex ${message.isMe ? 'justify-end' : 'justify-start'}`}>
                      <div className={`max-w-[75%] ${message.isMe ? 'bg-blue-500 text-white' : 'bg-white text-gray-800'} rounded-lg px-4 py-2 shadow-sm`}>
                        {/* Sender name for group chats */}
                        {activeConversation.isGroup && !message.isMe && (
                          <p className="text-xs font-medium mb-1" style={{ color: message.isMe ? 'white' : '#6366f1' }}>
                            {message.sender}
                          </p>
                        )}
                        ...
                        {/* Time and status */}
                        <div className={`flex items-center justify-end space-x-1 mt-1 text-xs ${message.isMe ? 'text-blue-100' : 'text-gray-500'}`}>
          ...
          <div className="flex-1 flex flex-col items-center justify-center p-4 bg-gray-50">
            ...
            <h3 className="text-xl font-semibold text-gray-900 mb-2">Chat Interno</h3>
        ...
        {inCall !== 'none' && (
          ...
          >
            <div className={`relative bg-gray-900 rounded-xl overflow-hidden ${
              isFullscreen ? 'w-full h-full' : 'max-w-2xl w-full'
            }`}>
              {/* Call Header */}
                ...
                <div className="flex items-center space-x-3 text-white">
                  {activeConversation?.avatar ? (
                    ...
                    <div className="w-10 h-10 rounded-full bg-blue-500 flex items-center justify-center">
                      <span className="text-lg font-semibold text-white">{activeConversation?.name.charAt(0)}</span>
                    </div>
                  )}
                  <div>
                ...
                {inCall === 'video' ? (
                  isVideoOff ? (
                    <div className="w-full h-full min-h-[300px] bg-gray-800 flex items-center justify-center">
                      <div className="text-center">
                        ...
                        <p className="text-white/70">Câmera desligada</p>
                      </div>
                    </div>
                  ) : (
                    // Simulated video feed
                    <div className="w-full h-full min-h-[300px] bg-gray-800 relative">
                      <img 
                  ...
                  <div className="text-center">
                    <div className="w-32 h-32 rounded-full bg-blue-500 flex items-center justify-center mx-auto mb-4">
                      <span className="text-5xl font-semibold text-white">{activeConversation?.name.charAt(0)}</span>
                    </div>
                    <h3 className="text-xl font-semibold text-white mb-2">{activeConversation?.name}</h3>
              ...
              {/* Call Controls */}
              <div className="absolute bottom-0 left-0 right-0 p-4 flex items-center justify-center space-x-4 z-10 bg-gradient-to-t from-black/60 to-transparent">
                <button 
          ...
          >
            <div className="flex items-center space-x-4 mb-4">
              <div className="w-12 h-12 rounded-full bg-blue-100 flex items-center justify-center">
                <span className="text-xl font-semibold text-blue-600">{callFrom.charAt(0)}</span>
              </div>
              <div>
                <h3 className="font-medium text-gray-900">{callFrom}</h3>
            ...
            <div className="flex justify-between">
              <button 
                onClick={declineCall}
                className="flex-1 py-2 bg-red-500 text-white rounded-lg mr-2 hover:bg-red-600 transition-colors"
              >
                Recusar
              </button>
              <button 
