/*
  # Corrigir Filtro de Data na View v_business_kpis

  ## Problema
  View está filtrando apenas últimos 30 dias, perdendo dados antigos

  ## Solução
  Aumentar período para 90 dias (3 meses) ou remover filtro
*/
...
WITH date_range AS (
  -- Expandir para 90 dias (3 meses) ao invés de 30
  SELECT 
...
finance_stats AS (
  SELECT
    -- Receitas (status = 'recebido' ou 'pago')
    COALESCE(SUM(valor) FILTER (WHERE tipo = 'receita' AND status IN ('recebido', 'pago')), 0) AS total_income,
    
    -- Despesas (status = 'pago')
    COALESCE(SUM(valor) FILTER (WHERE tipo = 'despesa' AND status = 'pago'), 0) AS total_expenses,
    
    -- A receber (receitas que não foram recebidas)
    COALESCE(SUM(valor) FILTER (WHERE tipo = 'receita' AND status NOT IN ('recebido', 'pago')), 0) AS receivable,
    
    -- A pagar (despesas com status 'a_pagar' ou 'pendente')
...
SELECT
  ...
  CASE 
    ...
    THEN (COALESCE(os.total_completed, 0)::NUMERIC / (COALESCE(os.total_completed, 0) + COALESCE(os.cancelled, 0))::NUMERIC * 100)
    ELSE 0
...
COMMENT ON VIEW v_business_kpis IS 'KPIs principais do negócio - últimos 90 dias para transações';
